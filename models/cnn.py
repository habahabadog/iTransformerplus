...
class Model(nn.Module):
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        self.conv_layers = nn.ModuleList([
            nn.Conv1d(configs.d_model, configs.d_model, kernel_size=3, padding=1, stride=1)
            for _ in range(configs.e_layers)
        ])
        self.residuals = [nn.Identity() for _ in range(configs.e_layers)]  # 根据需要可替换为更复杂的残差连接
    ...
    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None, mask=None):
        ...
        for conv, residual in zip(self.conv_layers, self.residuals):
            residual_x = enc_out
            enc_out = conv(enc_out)
            enc_out += residual(residual_x)  # 应用残差连接
